...
const listProjects = async () =>
  (await api.get<Project[]>("/api/projects")).data;
...
const updateProject = async (id: number, payload: any) =>
  (await api.patch(`/api/projects/${id}`, payload)).data;
...
export default function ProjectsPage() {
  ...
  async function onDelete(id: number) {
    if (
      !window.confirm(
        "정말로 이 프로젝트를 삭제하시겠습니까?\n관련된 모든 실험 데이터와 설정이 영구적으로 삭제됩니다."
      )
    )
      return;
  ...
  return (
    <div className="row" style={{ alignItems: "flex-start", gap: "24px" }}>
      ...
      <div
        className="card"
        style={{ flex: "0 0 320px", position: "sticky", top: "20px" }}
      >
      ...
      <div className="card" style={{ flex: "1", minWidth: "600px" }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                ...
                >
                  등록된 프로젝트가 없습니다. 왼쪽 폼에서 첫 프로젝트를 생성해
                  보세요.
                </td>
              </tr>
            ) : (
              items.map((p) => (
                <tr key={p.id}>
                  ...
                  <td style={{ textAlign: "center" }}>
                    <span
                      className={`badge ${
                        p.project_type === "VOC" ? "badge-voc" : "badge-regular"
                      }`}
                    >
        ...
        >
          <p className="small" style={{ margin: 0 }}>
            * <strong>상세/설정</strong> 버튼을 클릭하여 실험 항목(Schema)을
            정의하거나 리포트를 확인할 수 있습니다.
          </p>
        </div>
      </div>
    </div>
