...
const getExperiment = async (id: number) =>
  (await api.get<Experiment>(`/api/experiments/${id}`)).data;
const createExperiment = async (payload: any) =>
  (await api.post<Experiment>("/api/experiments", payload)).data;
const updateExperiment = async (id: number, payload: any) =>
  (await api.patch<Experiment>(`/api/experiments/${id}`, payload)).data;
...
export default function NewExperimentPage() {
  ...
  const sumAmounts = (arr: number[]) =>
    arr.reduce((sum, v) => sum + (Number(v) || 0), 0);

  const totalAmount = useMemo(() => {
    return materials.reduce((sum, m) => sum + sumAmounts(m.amount), 0);
  }, [materials]);

  function updateMaterial(idx: number, patch: Partial<MaterialLine>) {
    setMaterials((prev) => {
      const next = prev.map((m, i) => (i === idx ? { ...m, ...patch } : m));
      const newTotal = next.reduce((sum, m) => sum + sumAmounts(m.amount), 0);
      return next.map((m) => ({
        ...m,
        ratio: newTotal > 0 ? (sumAmounts(m.amount) / newTotal) * 100 : 0,
      }));
    });
  }
  ...
  async function onSave() {
    ...
    const cleanedMaterials = materials
      .map((m) => ({
        name: m.name,
        amount: (Array.isArray(m.amount) ? m.amount : [m.amount])
          .map((v) => Number(v))
          .filter((v) => !isNaN(v) && v > 0),
        unit: m.unit,
        ratio: Number(Number(m.ratio).toFixed(2)),
      }))
      .filter((m) => m.name.trim() !== "" && m.amount.length > 0);
    ...
    schemas.forEach((schema) => {
      ...
      if (schema.value_type === "quantitative") {
        ...
        cleanedResults[schema.key] =
          numeric.length === 1 ? numeric[0] : numeric;
        return;
      }
  ...
  return (
    <div
      className="card"
      style={{ maxWidth: "1000px", margin: "0 auto", boxSizing: "border-box" }}
    >
      <div
        className="row"
        style={{
              ...
              <tr key={idx}>
                ...
                <td>
                  <input
                    className="input"
                    type="text"
                    placeholder="10, 12.5, 9"
                    style={{ width: "100%" }}
                    value={
                      Array.isArray(m.amount) ? m.amount.join(", ") : m.amount
                    }
                    onChange={(e) => {
                      const numbers = e.target.value
                        ...
                        .filter((v) => !isNaN(v));
                      updateMaterial(idx, {
                        amount: numbers.length > 0 ? numbers : [],
                      });
                    }}
                  />
                </td>
                <td>
                  <select
                    ...
                    value={m.unit}
                    onChange={(e) =>
                      updateMaterial(idx, { unit: e.target.value as any })
                    }
                  >
              ...
              <div style={{ flex: 1 }}>
                {s.value_type === "quantitative" ? (
                  <input
                    className="input"
                    type="text"
                    placeholder="숫자 또는 쉼표로 구분된 값"
                    style={{ width: "100%" }}
                    value={
                      Array.isArray(results[s.key])
                        ? (results[s.key] as number[]).join(", ")
                        : results[s.key] ?? ""
                    }
                    onChange={(e) =>
                      setResults((prev) => ({
      ...
      <div
        className="row"
        style={{ marginTop: 30, gap: "15px", alignItems: "center" }}
      >
        <button
          className="btn"
