...
export async function updateResultSchema(
  schemaId: number,
  payload: Partial<
    Omit<ResultSchema, "id" | "project_id" | "created_at" | "key">
  >
) {
  const r = await api.patch<ResultSchema>(
    `/api/result-schemas/${schemaId}`,
    payload
  );
  return r.data;
}
...
export async function getOutputConfig(projectId: number) {
  const r = await api.get<OutputConfig | null>(
...
export async function listExperiments(projectId: number) {
  const r = await api.get<Experiment[]>(
...
export async function createExperiment(
  payload: Omit<Experiment, "id" | "created_at">
) {
...
export async function updateExperiment(
  ...
  const r = await api.patch<Experiment>(
